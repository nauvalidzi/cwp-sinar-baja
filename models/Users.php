...
class Users extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // id
        $this->id = new DbField('users', 'users', 'x_id', 'id', '`id`', '`id`', 3, 11, -1, false, '`id`', false, false, false, 'FORMATTED TEXT', 'NO');
        ...
        // fullname
        $this->fullname = new DbField('users', 'users', 'x_fullname', 'fullname', '`fullname`', '`fullname`', 200, 255, -1, false, '`fullname`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // username
        $this->_username = new DbField('users', 'users', 'x__username', 'username', '`username`', '`username`', 200, 20, -1, false, '`username`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // phone
        $this->phone = new DbField('users', 'users', 'x_phone', 'phone', '`phone`', '`phone`', 200, 15, -1, false, '`phone`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // nik
        $this->nik = new DbField('users', 'users', 'x_nik', 'nik', '`nik`', '`nik`', 200, 30, -1, false, '`nik`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // iddepartemen
        $this->iddepartemen = new DbField('users', 'users', 'x_iddepartemen', 'iddepartemen', '`iddepartemen`', '`iddepartemen`', 3, 11, -1, false, '`iddepartemen`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        $this->iddepartemen->IsForeignKey = true; // Foreign key field
        $this->iddepartemen->Nullable = false; // NOT NULL field
        $this->iddepartemen->Required = true; // Required field
        $this->iddepartemen->Sortable = true; // Allow sort
        $this->iddepartemen->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->iddepartemen->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        $this->iddepartemen->Lookup = new Lookup('iddepartemen', 'departemen', false, 'id', ["nama","","",""], [], [], [], [], [], [], '', '');
        $this->iddepartemen->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->iddepartemen->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->iddepartemen->Param, "CustomMsg");
        $this->Fields['iddepartemen'] = &$this->iddepartemen;
    }
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "departemen") {
            $detailUrl = Container("departemen")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_iddepartemen", $this->iddepartemen->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "UsersList";
        }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->iddepartemen->DbValue = $row['iddepartemen'];
    }
    ...
    public function getEditUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("UsersEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    public function getCopyUrl($parm = "")
    {
        if ($parm != "") {
            ...
            $url = $this->keyUrl("UsersAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
...
SORTHTML;
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->iddepartemen->setDbValue($row['iddepartemen']);
    }
    ...
    public function renderListRow()
    {
        ...
        // iddepartemen
        $curVal = trim(strval($this->iddepartemen->CurrentValue));
        if ($curVal != "") {
            $this->iddepartemen->ViewValue = $this->iddepartemen->lookupCacheOption($curVal);
            if ($this->iddepartemen->ViewValue === null) { // Lookup from database
                $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                $sqlWrk = $this->iddepartemen->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                if ($ari > 0) { // Lookup values found
                    $arwrk = $this->iddepartemen->Lookup->renderViewRow($rswrk[0]);
                    $this->iddepartemen->ViewValue = $this->iddepartemen->displayValue($arwrk);
                } else {
                    $this->iddepartemen->ViewValue = $this->iddepartemen->CurrentValue;
                }
            }
        } else {
            $this->iddepartemen->ViewValue = null;
        }
        $this->iddepartemen->ViewCustomAttributes = "";
        ...
        // iddepartemen
        $this->iddepartemen->LinkCustomAttributes = "";
        $this->iddepartemen->HrefValue = "";
        $this->iddepartemen->TooltipValue = "";
    ...
    public function renderEditRow()
    {
        ...
        // iddepartemen
        $this->iddepartemen->EditAttrs["class"] = "form-control";
        $this->iddepartemen->EditCustomAttributes = "";
        $this->iddepartemen->PlaceHolder = RemoveHtml($this->iddepartemen->caption());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->iddepartemen);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->fullname);
                    $doc->exportCaption($this->_email);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->iddepartemen);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->fullname);
                        $doc->exportField($this->_email);
