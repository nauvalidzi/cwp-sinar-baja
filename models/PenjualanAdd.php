 ...
 * Page class
 */
class PenjualanAdd extends Penjualan
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenjualanAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PenjualanList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PenjualanList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PenjualanView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("penjualan_detail", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("PenjualanDetailGrid");
            if (in_array("penjualan_detail", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->idpenjualan->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "penjualan_detail"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->idpenjualan->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
