 ...
 * Page class
 */
class PenjualanDetailGrid extends PenjualanDetail
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PenjualanDetailGrid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fpenjualan_detailgrid";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "PenjualanDetailAdd";
    ...
    public function run()
    {
        ...
        $this->idpenjualan->Visible = false;
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "penjualan") {
            $masterTbl = Container("penjualan");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("PenjualanList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = ROWTYPE_MASTER; // Master row
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "penjualan") {
            $this->idpenjualan->CurrentValue = $this->idpenjualan->getSessionValue();
        }
        $conn = $this->getConnection();
        ...
        if ($this->idpenjualan->getSessionValue() != "") {
            $rsnew['idpenjualan'] = $this->idpenjualan->getSessionValue();
        }
