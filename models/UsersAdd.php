...
class UsersAdd extends Users
{
    ...
    public function run()
    {
        ...
        $this->iddepartemen->setVisibility();
        ...
        $this->setupLookupOptions($this->iddepartemen);
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->iddepartemen->CurrentValue = null;
        $this->iddepartemen->OldValue = $this->iddepartemen->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'iddepartemen' first before field var 'x_iddepartemen'
        $val = $CurrentForm->hasValue("iddepartemen") ? $CurrentForm->getValue("iddepartemen") : $CurrentForm->getValue("x_iddepartemen");
        if (!$this->iddepartemen->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->iddepartemen->Visible = false; // Disable update for API request
            } else {
                $this->iddepartemen->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->iddepartemen->CurrentValue = $this->iddepartemen->FormValue;
    }
    ...
    protected function newRow()
    {
        ...
        $row['iddepartemen'] = $this->iddepartemen->CurrentValue;
        return $row;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->iddepartemen->ViewValue = $this->iddepartemen->Lookup !== null && is_array($this->iddepartemen->Lookup->Options) ? $curVal : null;
            }
            if ($this->iddepartemen->ViewValue !== null) { // Load from cache
                $this->iddepartemen->EditValue = array_values($this->iddepartemen->Lookup->Options);
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = "`id`" . SearchString("=", $this->iddepartemen->CurrentValue, DATATYPE_NUMBER, "");
                }
                $sqlWrk = $this->iddepartemen->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->iddepartemen->EditValue = $arwrk;
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->iddepartemen->Required) {
            if (!$this->iddepartemen->IsDetailKey && EmptyValue($this->iddepartemen->FormValue)) {
                $this->iddepartemen->addErrorMessage(str_replace("%s", $this->iddepartemen->caption(), $this->iddepartemen->RequiredErrorMessage));
            }
        }

        // Validate detail grid
        $detailTblVar = explode(",", $this->getCurrentDetailTable());
        $detailPage = Container("DepartemenGrid");
        if (in_array("departemen", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // iddepartemen
        $this->iddepartemen->setDbValueDef($rsnew, $this->iddepartemen->CurrentValue, 0, false);
        ...
        if ($addRow) {
            ...
            $detailPage = Container("DepartemenGrid");
            if (in_array("departemen", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->id->setSessionValue($this->iddepartemen->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "departemen"); // Load user level of detail table
    ...
    protected function setupDetailParms()
    {
        ...
        if ($detailTblVar != "") {
            ...
            if (in_array("departemen", $detailTblVar)) {
                $detailPageObj = Container("DepartemenGrid");
                if ($detailPageObj->DetailAdd) {
                    ...
                    $detailPageObj->id->CurrentValue = $this->iddepartemen->CurrentValue;
