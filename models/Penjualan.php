...
class Penjualan extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // tanggal
        $this->tanggal = new DbField('penjualan', 'penjualan', 'x_tanggal', 'tanggal', '`tanggal`', CastDateFieldForLike("`tanggal`", 7, "DB"), 135, 19, 7, false, '`tanggal`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // kode
        $this->kode = new DbField('penjualan', 'penjualan', 'x_kode', 'kode', '`kode`', '`kode`', 200, 20, -1, false, '`kode`', false, false, false, 'FORMATTED TEXT', 'TEXT');
        ...
        // region
        $this->region = new DbField('penjualan', 'penjualan', 'x_region', 'region', '`region`', '`region`', 200, 100, -1, false, '`region`', false, false, false, 'FORMATTED TEXT', 'SELECT');
        ...
        $this->region->Lookup = new Lookup('region', 'penjualan', false, '', ["","","",""], [], [], [], [], [], [], '', '');
        $this->region->OptionCount = 7;
        $this->region->CustomMsg = $Language->FieldPhrase($this->TableVar, $this->region->Param, "CustomMsg");
        $this->Fields['region'] = &$this->region;

        // total
        $this->total = new DbField('penjualan', 'penjualan', 'x_total', 'total', '`total`', '`total`', 5, 10, -1, false, '`total`', false, false, false, 'FORMATTED TEXT', 'TEXT');
    ...
    public function getDetailUrl()
    {
        // Detail url
        $detailUrl = "";
        if ($this->getCurrentDetailTable() == "penjualan_detail") {
            $detailUrl = Container("penjualan_detail")->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
            $detailUrl .= "&" . GetForeignKeyUrl("fk_id", $this->id->CurrentValue);
        }
        if ($detailUrl == "") {
            $detailUrl = "PenjualanList";
        }
        return $detailUrl;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("PenjualanList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "PenjualanEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "PenjualanAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PenjualanView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PenjualanView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "PenjualanAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "PenjualanAdd";
        }
        return $this->addMasterUrl($url);
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PenjualanEdit", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PenjualanEdit", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("PenjualanAdd", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("PenjualanAdd", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("PenjualanDelete", $this->getUrlParm());
    }
...
SORTHTML;
    ...
    public function renderEditRow()
    {
        ...
        $this->region->EditValue = $this->region->options(true);
