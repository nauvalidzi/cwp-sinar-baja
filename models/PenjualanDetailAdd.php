 ...
 * Page class
 */
class PenjualanDetailAdd extends PenjualanDetail
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'penjualan_detail';

    // Page object name
    public $PageObjName = "PenjualanDetailAdd";
    ...
    public function __construct()
    {
        ...
        // Table object (penjualan_detail)
        if (!isset($GLOBALS["penjualan_detail"]) || get_class($GLOBALS["penjualan_detail"]) == PROJECT_NAMESPACE . "penjualan_detail") {
            $GLOBALS["penjualan_detail"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'penjualan_detail');
        }
    ...
    public function terminate($url = "")
    {
        ...
        if ($this->CustomExport && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, Config("EXPORT_CLASSES"))) {
            ...
            if (class_exists($class)) {
                $doc = new $class(Container("penjualan_detail"));
                $doc->Text = @$content;
    ...
    public function run()
    {
        ...
        $this->id->Visible = false;
        $this->idpenjualan->setVisibility();
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->idpenjualan);
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("PenjualanDetailList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "PenjualanDetailList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "PenjualanDetailView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->idpenjualan->CurrentValue = null;
        $this->idpenjualan->OldValue = $this->idpenjualan->CurrentValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idpenjualan' first before field var 'x_idpenjualan'
        $val = $CurrentForm->hasValue("idpenjualan") ? $CurrentForm->getValue("idpenjualan") : $CurrentForm->getValue("x_idpenjualan");
        if (!$this->idpenjualan->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->idpenjualan->Visible = false; // Disable update for API request
            } else {
                $this->idpenjualan->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->idpenjualan->CurrentValue = $this->idpenjualan->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row['idpenjualan'] = $this->idpenjualan->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->idpenjualan->getSessionValue() != "") {
                ...
                $this->idpenjualan->EditValue = HtmlEncode($this->idpenjualan->CurrentValue);
                $curVal = trim(strval($this->idpenjualan->CurrentValue));
                if ($curVal != "") {
                    $this->idpenjualan->EditValue = $this->idpenjualan->lookupCacheOption($curVal);
                    if ($this->idpenjualan->EditValue === null) { // Lookup from database
                        $filterWrk = "`id`" . SearchString("=", $curVal, DATATYPE_NUMBER, "");
                        $sqlWrk = $this->idpenjualan->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                        $rswrk = Conn()->executeQuery($sqlWrk)->fetchAll(\PDO::FETCH_BOTH);
                        $ari = count($rswrk);
                        if ($ari > 0) { // Lookup values found
                            $arwrk = $this->idpenjualan->Lookup->renderViewRow($rswrk[0]);
                            $this->idpenjualan->EditValue = $this->idpenjualan->displayValue($arwrk);
                        } else {
                            $this->idpenjualan->EditValue = HtmlEncode($this->idpenjualan->CurrentValue);
                        }
                    }
                } else {
                    $this->idpenjualan->EditValue = null;
                }
    ...
    protected function validateForm()
    {
        ...
        if ($this->idpenjualan->Required) {
            if (!$this->idpenjualan->IsDetailKey && EmptyValue($this->idpenjualan->FormValue)) {
                $this->idpenjualan->addErrorMessage(str_replace("%s", $this->idpenjualan->caption(), $this->idpenjualan->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->idpenjualan->FormValue)) {
            $this->idpenjualan->addErrorMessage($this->idpenjualan->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // idpenjualan
        $this->idpenjualan->setDbValueDef($rsnew, $this->idpenjualan->CurrentValue, 0, false);
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "penjualan") {
                $validMaster = true;
                $masterTbl = Container("penjualan");
                if (($parm = Get("fk_id", Get("idpenjualan"))) !== null) {
                    $masterTbl->id->setQueryStringValue($parm);
                    $this->idpenjualan->setQueryStringValue($masterTbl->id->QueryStringValue);
                    $this->idpenjualan->setSessionValue($this->idpenjualan->QueryStringValue);
                    if (!is_numeric($masterTbl->id->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
            ...
            if ($masterTblVar == "penjualan") {
                $validMaster = true;
                $masterTbl = Container("penjualan");
                if (($parm = Post("fk_id", Post("idpenjualan"))) !== null) {
                    $masterTbl->id->setFormValue($parm);
                    $this->idpenjualan->setFormValue($masterTbl->id->FormValue);
                    $this->idpenjualan->setSessionValue($this->idpenjualan->FormValue);
                    if (!is_numeric($masterTbl->id->FormValue)) {
                        $validMaster = false;
                    }
                } else {
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "penjualan") {
                if ($this->idpenjualan->CurrentValue == "") {
                    $this->idpenjualan->setSessionValue("");
                }
            }
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("PenjualanDetailList"), "", $this->TableVar, true);
