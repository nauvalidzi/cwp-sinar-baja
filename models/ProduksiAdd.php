 ...
 * Page class
 */
class ProduksiAdd extends Produksi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProduksiAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("ProduksiList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ProduksiList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ProduksiView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("produksi_detail", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("ProduksiDetailGrid");
            if (in_array("produksi_detail", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->idproduksi->setSessionValue($this->id->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "produksi_detail"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->idproduksi->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
