...
// Page object
$PenjualanDetailAdd = &$Page;
?>
<script>
var currentForm, currentPageID;
var fpenjualan_detailadd;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "add";
    fpenjualan_detailadd = currentForm = new ew.Form("fpenjualan_detailadd", "add");
    ...
    fpenjualan_detailadd.addFields([
        ["idpenjualan", [fields.idpenjualan.visible && fields.idpenjualan.required ? ew.Validators.required(fields.idpenjualan.caption) : null, ew.Validators.integer], fields.idpenjualan.isInvalid],
    ...
    // Validate form
    fpenjualan_detailadd.validate = function () {
    ...
    // Form_CustomValidate
    fpenjualan_detailadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpenjualan_detailadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpenjualan_detailadd.lists.idpenjualan = <?= $Page->idpenjualan->toClientList($Page) ?>;
    fpenjualan_detailadd.lists.idproduk = <?= $Page->idproduk->toClientList($Page) ?>;
    loadjs.done("fpenjualan_detailadd");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fpenjualan_detailadd" id="fpenjualan_detailadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->idpenjualan->Visible) { // idpenjualan ?>
    <div id="r_idpenjualan" class="form-group row">
        <label id="elh_penjualan_detail_idpenjualan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idpenjualan->caption() ?><?= $Page->idpenjualan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->idpenjualan->cellAttributes() ?>>
<?php if ($Page->idpenjualan->getSessionValue() != "") { ?>
<span id="el_penjualan_detail_idpenjualan">
<span<?= $Page->idpenjualan->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idpenjualan->getDisplayValue($Page->idpenjualan->ViewValue))) ?>"></span>
</span>
<input type="hidden" id="x_idpenjualan" name="x_idpenjualan" value="<?= HtmlEncode($Page->idpenjualan->CurrentValue) ?>" data-hidden="1">
<?php } else { ?>
<span id="el_penjualan_detail_idpenjualan">
<?php
$onchange = $Page->idpenjualan->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$Page->idpenjualan->EditAttrs["onchange"] = "";
?>
<span id="as_x_idpenjualan" class="ew-auto-suggest">
    <input type="<?= $Page->idpenjualan->getInputTextType() ?>" class="form-control" name="sv_x_idpenjualan" id="sv_x_idpenjualan" value="<?= RemoveHtml($Page->idpenjualan->EditValue) ?>" size="30" placeholder="<?= HtmlEncode($Page->idpenjualan->getPlaceHolder()) ?>" data-placeholder="<?= HtmlEncode($Page->idpenjualan->getPlaceHolder()) ?>"<?= $Page->idpenjualan->editAttributes() ?> aria-describedby="x_idpenjualan_help">
</span>
<input type="hidden" is="selection-list" class="form-control" data-table="penjualan_detail" data-field="x_idpenjualan" data-input="sv_x_idpenjualan" data-value-separator="<?= $Page->idpenjualan->displayValueSeparatorAttribute() ?>" name="x_idpenjualan" id="x_idpenjualan" value="<?= HtmlEncode($Page->idpenjualan->CurrentValue) ?>"<?= $onchange ?>>
<?= $Page->idpenjualan->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->idpenjualan->getErrorMessage() ?></div>
<script>
loadjs.ready(["fpenjualan_detailadd"], function() {
    fpenjualan_detailadd.createAutoSuggest(Object.assign({"id":"x_idpenjualan","forceSelect":false}, ew.vars.tables.penjualan_detail.fields.idpenjualan.autoSuggestOptions));
});
</script>
<?= $Page->idpenjualan->Lookup->getParamTag($Page, "p_x_idpenjualan") ?>
</span>
<?php } ?>
</div></div>
    </div>
