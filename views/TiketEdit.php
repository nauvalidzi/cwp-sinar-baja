...
loadjs.ready("head", function () {
    ...
    ftiketedit = currentForm = new ew.Form("ftiketedit", "edit");
    ...
    // Form_CustomValidate
    ftiketedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    ftiketedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    ftiketedit.lists.subjek = <?= $Page->subjek->toClientList($Page) ?>;
    ftiketedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("ftiketedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="ftiketedit" id="ftiketedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<script>
loadjs.ready(["ftiketedit"], function() {
    ftiketedit.createAutoSuggest(Object.assign({"id":"x_subjek","forceSelect":true}, ew.vars.tables.tiket.fields.subjek.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["ftiketedit", "datetimepicker"], function() {
    ew.createDateTimePicker("ftiketedit", "x_tanggal", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<?php } ?>
    ...
    <input type="hidden" data-table="tiket" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
<?php
    if (in_array("tiket_detail", explode(",", $Page->getCurrentDetailTable())) && $tiket_detail->DetailEdit) {
?>
