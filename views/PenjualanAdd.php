...
loadjs.ready("head", function () {
    ...
    // Form_CustomValidate
    fpenjualanadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fpenjualanadd.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fpenjualanadd.lists.region = <?= $Page->region->toClientList($Page) ?>;
...
$Page->showMessage();
?>
<form name="fpenjualanadd" id="fpenjualanadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
loadjs.ready(["fpenjualanadd", "datetimepicker"], function() {
    ew.createDateTimePicker("fpenjualanadd", "x_tanggal", {"ignoreReadonly":true,"useCurrent":false,"format":7});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->kode->Visible) { // kode ?>
    <div id="r_kode" class="form-group row">
        <label id="elh_penjualan_kode" for="x_kode" class="<?= $Page->LeftColumnClass ?>"><?= $Page->kode->caption() ?><?= $Page->kode->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->kode->cellAttributes() ?>>
<span id="el_penjualan_kode">
<input type="<?= $Page->kode->getInputTextType() ?>" data-table="penjualan" data-field="x_kode" name="x_kode" id="x_kode" size="30" maxlength="20" placeholder="<?= HtmlEncode($Page->kode->getPlaceHolder()) ?>" value="<?= $Page->kode->EditValue ?>"<?= $Page->kode->editAttributes() ?> aria-describedby="x_kode_help">
...
<?php
    if (in_array("penjualan_detail", explode(",", $Page->getCurrentDetailTable())) && $penjualan_detail->DetailAdd) {
?>
